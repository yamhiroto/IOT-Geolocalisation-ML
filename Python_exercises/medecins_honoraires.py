...
"""
07/11/2019
Using data from medical websites, we want to analyze the link between doctor payments
and their population by specialty
"""
...
def buildAndGetMatchingMap(col_1, col_2, deg):
    a={}
    for spe_1 in col_1:
        for spe_2 in col_2:
...
# Following function looks more elegant (no double loop) however it takes much more time to compute
...
# Loading extra-fess per specialty
...
# Loading number of doctors per specialty
...
# Build matching map for dataframe 1
a=buildAndGetMatchingMap(list(df_1['l_pre_spe'].unique()), df_2['specialites'], 0.7)
# Some specialty has no corresponding value from our matching function;
# We fill the empty cells with human intuition
